...
| **T-03: Stolen Agent Credentials** | Attacker extracts `device_id` and `device_secret` from a user host machine. | HIGH | 1. **Device Isolation**: Credentials are valid **only** for that single `device_id`. Attacker cannot control other devices or read other tenant data.<br>2. **HMAC Signing Requirement**: Secret cannot be used without valid timestamps, nonces, and request signatures.<br>3. **Instant Revocation**: Tenant admin can revoke or delete the device key via Backend API, immediately blocking WSS streams. |
